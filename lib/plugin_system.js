...
// New Plugin class
class Plugin {
    constructor(pluginModule, oj, db, token, permissions) {
    ...
    executeHook(hookName, ...args) {
        if (typeof this.module[hookName] === 'function') {
            // Pass the permission-aware proxies to the plugin
            return this.module[hookName](this.ojProxy, this.dbProxy, ...args);
        }
        return null;
    }
}
    ...
    async loadAll() {
        ...
        for (const file of files) {
            ...
            try {
                ...
                const wrappedPlugin = new Plugin(pluginModule, this.oj, this.db, token, grantedPermissions);
